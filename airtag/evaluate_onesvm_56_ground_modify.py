...
suffix=args.suffix+"/" # suffix = automationmul/

start_load_train=time.time()

from pathlib import Path
base_dir = Path(__file__).resolve().parent
embedding_path = base_dir / "embedding_data" / (suffix + f"M56_benign.json")
...
base_dir = Path(__file__).resolve().parent
embedding_path = base_dir / "embedding_data" / (suffix + f"M561_test.json")
...
strr = np.load(base_dir / "ground_data" / (suffix + "M5h1_number.npy"))
strr=list(strr)
...
log_file = base_dir / "training_data" / f"{suffix}training_preprocessed_logs_M5-CVE-2017-0199_windows_py_h1"

start_cali_time = time.time()
predict_labels = second_class(str(log_file), predict_labels, float(args.thres))
...
start_load_test_time=time.time()
base_dir = Path(__file__).resolve().parent
embedding_path = base_dir / "embedding_data" / (suffix + f"M562_test.json")
f = open(embedding_path, 'r')
strr=f.read().split("\n")
f.close()
for i in range(len(strr)-1):
...
print("end load test")
#print(end_load_test_time-start_load_test_time)
strr = np.load(base_dir / "ground_data" / (suffix + "M5h2_number.npy"))
strr=list(strr)
labels=np.ones(len(value2))
for i in range(len(strr)):
  if int(strr[i])<=len(value2):
...
log_file = base_dir / "training_data" / f"{suffix}training_preprocessed_logs_M5-CVE-2017-0199_windows_py_h2"

start_cali_time = time.time()
predict_labels = second_class(str(log_file), predict_labels)
end_cali_time = time.time()
#print("end cali  time")
#print(end_cali_time-start_cali_time)
a1=0
...
start_load_test_time=time.time()
base_dir = Path(__file__).resolve().parent
embedding_path = base_dir / "embedding_data" / (suffix + f"M563_test.json")
f = open(embedding_path, 'r')
strr=f.read().split("\n")
f.close()
for i in range(len(strr)-1):
...
print("end load test")
#print(end_load_test_time-start_load_test_time)
strr = np.load(base_dir / "ground_data" / (suffix + "M6h1_number.npy"))
strr=list(strr)
labels=np.ones(len(value2))
for i in range(len(strr)):
  if int(strr[i])<=len(value2):
...
log_file = base_dir / "training_data" / f"{suffix}training_preprocessed_logs_M6-CVE-2017-11882_windows_h1"
...
start_load_test_time=time.time()
base_dir = Path(__file__).resolve().parent
embedding_path = base_dir / "embedding_data" / (suffix + f"M564_test.json")
f = open(embedding_path, 'r')
strr=f.read().split("\n")
f.close()
for i in range(len(strr)-1):
...
#print(end_load_test_time-start_load_test_time)
value2=np.array(strr[:-1])
strr = np.load(base_dir / "ground_data" / (suffix + "M6h2_number.npy"))
strr=list(strr)
labels=np.ones(len(value2))
for i in range(len(strr)):
  if int(strr[i])<=len(value2):
...
log_file = base_dir / "training_data" / f"{suffix}training_preprocessed_logs_M6-CVE-2017-11882_windows_h2"
